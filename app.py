...
def main_keyboard() -> str:
    return inline_keyboard(
        [
            [
                {"text": "⬇️ Download YouTube", "callback_data": "mode:download", "style": "primary"},
...
def platform_keyboard() -> str:
    ...
    return inline_keyboard(
        [
            [{"text": "▶️ YouTube", "callback_data": "platform:youtube", "style": "primary"}],
...
def youtube_info(source_url: str) -> tuple[str, list[dict[str, Any]]]:
    """Resolve public YouTube media through yt-dlp."""
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
        'format': 'best',
    ...
    if not options:
        raise RuntimeError("ইউটিউব থেকে ডাউনলোডের উপযোগী কোনো ভিডিও লিংক পাওয়া যায়নি।")
...
def download_youtube(
    ...
    if not media_url:
        raise RuntimeError("মিডিয়া URL পাওয়া যায়নি।")
...
def send_download(chat_id: int, status_id: int, source_url: str, option: dict[str, Any]) -> None:
    ...
    def update(downloaded: int, total: int, stage: str) -> None:
        ...
        edit_message(
            chat_id,
            status_id,
            progress_text(str(option.get("title", "YouTube Video")), downloaded, total, stage),
        )

    try:
        output_format = str(option["output_format"])
        media_url, title, extension = download_youtube(
            source_url,
            option,
            output_format,
            update,
...
def resolve_download(chat_id: int, status_id: int, source_url: str) -> None:
    try:
        ...
        edit_message(
            chat_id,
            status_id,
            f"<b>YouTube video resolve করা যায়নি</b>\n\n{escape(str(error))}",
            reply_markup=platform_keyboard(),
        )
...
def welcome_text(first_name: str = "") -> str:
    ...
    return (
        ...
        "Public YouTube video থেকে MP4 বা MP3 তৈরি করে Telegram-এ পাঠান। "
...
def help_text() -> str:
    return (
        ...
        "১. <b>Download YouTube</b> চাপুন\n"
        "২. Public YouTube URL পাঠান\n"
        "৩. Quality বেছে নিন\n"
        ...
        "Private বা age-restricted content সাপোর্ট করবে না। "
        "শুধু public content লিঙ্ক ব্যবহার করুন।"
    )
...
def process_message(message: dict[str, Any]) -> None:
    ...
    if command == "/ai":
        ...
        if prompt:
            ...
            send_message(chat_id, "AI mode चालू। আপনার প্রশ্ন লিখুন।")
        return
    if command == "/image":
        prompt = text[len(command) :].strip()
        if not prompt:
    ...
    if mode == "platform":
        send_message(chat_id, "আগে YouTube বেছে নিন।", reply_markup=platform_keyboard())
        return
    if mode == "awaiting_url":
        ...
        if not url or not platform or not detected or detected.key != platform.key:
            send_message(
                chat_id,
                "এটি valid public YouTube URL মনে হচ্ছে না। আবার URL পাঠান।",
                reply_markup=platform_keyboard(),
            )
            return
        with STATE_LOCK:
    ...
    if mode == "ai":
        ...
        return
    send_message(chat_id, "একটি mode বেছে নিন—YouTube download বা AI assistant।", reply_markup=main_keyboard())
...
def process_callback(callback: dict[str, Any]) -> None:
    ...
    if data == "home":
        ...
        CHAT_PLATFORMS[chat_id] = "youtube"
        edit_message(
            chat_id,
            message_id,
            "<b>▶️ YouTube selected</b>\n\nএখন public video URL পাঠান।",
